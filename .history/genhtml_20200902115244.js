...
function distributeLessons() {
  database.map((item) => {
    let setLesson = `${item.account === "SP" ? (browserAndVersion.includes("Chrome") ? "browserColor" : "") : ""}
    ...
    if (item.current !== true) {
      ...
      document.getElementById(`lessons${item.subject}`).innerHTML += `
        <div class="lesson lesson-background-filter ${setLesson}
        ">
          <a class="lessonname" href=${item.url}>${item.title}</a>
          <span class="hours">${item.hours}</span>
          <span class="account ${setAccount}"></span>
        </div>
    `
    } else {
      document.getElementById(`lessons${item.subject}`).innerHTML += `
